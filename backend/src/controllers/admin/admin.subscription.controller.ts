...
export class AdminSubscriptionPlanController {
  ...
  async createPlan(req: Request, res: Response): Promise<void> {
    const plan = await this._planService.create(req.body);
  ...
  async getPlanById(req: Request, res: Response): Promise<void> {
    const plan = await this._planService.getById(req.params.id);
    if (!plan) {
  ...
  async updatePlan(req: Request, res: Response): Promise<void> {
    const updated = await this._planService.update(req.params.id, req.body);
    sendResponse(res, STATUS_CODES.OK, MESSAGES.SUBSCRIPTION_PLAN_UPDATED, true, updated);
  }

  async deletePlan(req: Request, res: Response): Promise<void> {
    await this._planService.delete(req.params.id);
