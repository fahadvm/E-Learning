...
import { GoogleLoginButton } from "@/components/employee/googleLogin";
...
export default function StudentSignupPage() {
  ...
  const handleSignup = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    setIsLoading(true);
    try {
      const response = await employeeApiMethods.signup(formData);
    
      if (response?.ok) {
        localStorage.setItem("tempSignupEmail", formData.email);
        showSuccessToast(response.message);
        router.push("/employee/signup/verify-otp");
      } else {
