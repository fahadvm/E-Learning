...
function generateTimeRanges() {
  ...
  for (let h = 0; h < 24; h++) {
    for (let m = 0; m < 60; m += 30) {
      ...
      ranges.push(
        `${startH}:${startM} - ${String(endH).padStart(2, "0")}:${String(
          endM
        ).padStart(2, "0")}`
      );
    }
...
export default function AvailabilityScheduler() {
  ...
  const getAvailableRangesForDay = (
    daySlots: TimeSlot[],
    ...
    return timeRanges.filter((r) => !used.includes(r));
  };

  const getFirstAvailableRange = (daySlots: TimeSlot[]) => {
    const used = daySlots.map((s) => `${s.start} - ${s.end}`);
    return timeRanges.find((r) => !used.includes(r));
  };
  ...
  const addSlot = (day: string) => {
    setWeek((prev) =>
      prev.map((d) => {
        if (d.day !== day || d.slots.length >= 4) return d;

        const range = getFirstAvailableRange(d.slots);
        if (!range) return d;
  ...
  const onSave = async () => {
    try {
      ...
      if (res.ok) showSuccessToast("Availability saved successfully");
    } catch (err) {
      console.error(err);
    }
  };
      ...
      <Card>
        <CardContent className="p-6">
          ...
          <p className="text-sm text-muted-foreground">
            Enable your availability by day. Add multiple time ranges.
          </p>
          ...
          <div className="mt-4 divide-y border rounded-lg">
            {week.map((d) => (
              <div
                key={d.day}
                className="grid grid-cols-1 gap-4 p-4 md:grid-cols-12"
              >
                <div className="md:col-span-3 flex items-center gap-3">
                  <Switch
                    checked={d.enabled}
                    onCheckedChange={(checked) =>
                      toggleDay(d.day, checked)
                    }
                    className="data-[state=checked]:bg-violet-500 data-[state=unchecked]:bg-black"
                  />
                ...
                <div className="md:col-span-9">
                  {d.enabled ? (
                    ...
                    <div className="border border-dashed p-4 text-sm text-muted-foreground">
                      Availability disabled for {d.day}
                    </div>
                  )}
                </div>
              </div>
